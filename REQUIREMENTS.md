...
| **Env override**                 | Support at least one of: **`VIBE_SQUIRE_DATA_DIR`** (directory; app creates it and uses a fixed filename inside) and/or **`DATABASE_PATH`** (full path to the `.db` file). Precedence: **env →** then default paths above.         |
